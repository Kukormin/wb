...
if (!function_exists('debugmessage'))
{
	function debugmessage($message, $title = false, $color = '#008B8B')
	{
		...
		<tr>
			<td style="color:<?= $color ?>;font-size:11px;font-family:Verdana;text-align:left;"><?
				if (strlen($title))
				{
					?><p>[<?= $title ?>]</p><?
				}
...
{
	function log_array($arFields = [], $sFileName = 'log.txt', $bBacktrace = true)
	{
...
{
	function _log_array($arFields = [], $sFileName = 'log.txt', $bBacktrace = true)
	{
		$sMess = '';
